...
// --- PLATEAU LOD1 buildings (preferred when available) ---
// Generated by scripts/fetch_plateau.py. Accurate measured heights/footprints;
// names and building types are transferred from the nearest OSM building.
let buildingSource = "OpenStreetMap";
let finalBuildings = buildings;
const plateauPath = join(OUT, "plateau_buildings.json");
if (existsSync(plateauPath)) {
  const plateau = JSON.parse(readFileSync(plateauPath, "utf8"));
  const converted = [];
  for (const pb of plateau.buildings) {
    const f = pb.ring.map(([lat, lon]) => toLocal(lon, lat));
    if (f.length < 3) continue;
    if (f[0][0] === f[f.length - 1][0] && f[0][1] === f[f.length - 1][1]) f.pop();
    if (f.length < 3 || dist(centroid(f)) > CLIP_BUILDING) continue;
    converted.push({ f, h: r2(pb.h || 8), t: "yes" });
  }
  if (converted.length >= 300) {
    const tagged = buildings
      .filter((b) => b.n || (b.t && b.t !== "yes"))
      .map((b) => ({ c: centroid(b.f), n: b.n, t: b.t }));
    for (const b of converted) {
      const c = centroid(b.f);
      let best = null;
      let bestD = 12 * 12; // match within 12 m
      for (const o of tagged) {
        const d = (o.c[0] - c[0]) ** 2 + (o.c[1] - c[1]) ** 2;
        if (d < bestD) { bestD = d; best = o; }
      }
      if (best) {
        if (best.n) b.n = best.n;
        if (best.t && best.t !== "yes") b.t = best.t;
      }
    }
    finalBuildings = converted;
    buildingSource = "Project PLATEAU (LOD1) + OpenStreetMap (names)";
    console.error(`using PLATEAU buildings: ${converted.length} (OSM fallback: ${buildings.length})`);
  } else {
    console.error(`PLATEAU data too sparse (${converted.length}), keeping OSM buildings`);
  }
}
...
const city = {
  meta: {
    ...
    source: `roads/rails/areas/POIs: OpenStreetMap via Overpass API; buildings: ${buildingSource}`,
    license:
      "ODbL 1.0 — (c) OpenStreetMap contributors — https://www.openstreetmap.org/copyright; " +
      "buildings may include PLATEAU data — 出典: 国土交通省 Project PLATEAU (編集・加工して使用)",
  },
  buildings: finalBuildings, roads, rails, platforms, green, water, pois,
};

// sanity checks — fail loudly rather than publishing a broken/empty city
const fail = (msg) => { console.error(`ERROR: ${msg}`); process.exit(1); };
if (finalBuildings.length < 300) fail(`too few buildings: ${finalBuildings.length}`);
...
writeFileSync(
  ...
  JSON.stringify({ ...fetchMeta, processed: city.meta.generated, counts: {
    buildings: finalBuildings.length, roads: roads.length, rails: rails.length,
...
console.log(`buildings=${finalBuildings.length} roads=${roads.length} rails=${rails.length} platforms=${platforms.length} green=${green.length} water=${water.length} pois=${pois.length}`);
